...
export default function ImageDialoguePage() {
  ...
  const fetchDialogue = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setMessages(data.dialogue.messages || []);
      }
    } catch (error) {
  ...
  const handleSendMessage = async () => {
    if (!prompt.trim() || !dialogueId) return;
    ...
    try {
      ...
      const dialogueResponse = await fetch(`/api/dialogues/${dialogueId}`);
